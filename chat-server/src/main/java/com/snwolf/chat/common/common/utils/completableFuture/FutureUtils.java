...
/**
 * CompletableFuture封装工具类, 来源自<a href="https://mp.weixin.qq.com/s/GQGidprakfticYnbVYVYGQ">美团技术文章</a>
 */
@Slf4j
public class FutureUtils {
     ...
     * 多用于分页查询的场景
     * <p>当前有多个任务, 每个任务的执行结果都是List<T>, 现在想要将这多个任务的结果收集到一个List<T>中, 这就是这个方法的作用
     */
    public static <T> CompletableFuture<List<T>> sequenceList(Collection<CompletableFuture<List<T>>> completableFutures) {
        return CompletableFuture.allOf(completableFutures.toArray(new CompletableFuture<?>[0]))
                .thenApply(v -> completableFutures.stream()
                        // 将每一个completableFutures中的结果List<T>继续拆分成多个stream流, 然后再统一收集到List<T>中
                        // listFuture.join()作用是获取每一个listFuture的返回值, 注意这里join()不会阻塞, 因为只有所有CompletableFuture都执行完成, 才会回调allOf()方法
